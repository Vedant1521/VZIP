...
bit_reader* br_init(FILE* fp) {
	bit_reader* new = malloc(sizeof(bit_reader));
	if (!new) return NULL;
	new->fp = fp;
	new->buffer = 0;
	new->count = 0;
...
static uint8_t read_bit(bit_reader* br) {
	assert(br && br->fp);
	if (br->count == 0) {
	...
	uint8_t bit = (br->buffer >> (br->count - 1)) & 1;
	br->count--;
	return bit;
}
